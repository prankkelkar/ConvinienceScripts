...
"ubuntu-16.04")
	...
	if [ "$USEAS" = "server" ]; then
		apt-get install -y wget zip unzip tar git g++ make rake libreadline6 libreadline6-dev openssl libyaml-dev libssl-dev libsqlite3-dev libc6-dev cron locales ant zip |& tee -a "$LOG_FILE"
	elif [ "$USEAS" = "agent" ]; then
