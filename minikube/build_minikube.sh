...
function gettingStarted() {
    ...
    printf -- "curl -Lo kubectl https://storage.googleapis.com/kubernetes-release/release/v1.15.4/bin/linux/s390x/kubectl && chmod +x kubectl && sudo cp kubectl /usr/local/bin/ && rm kubectl \n"
    ...
    printf -- "sudo -E minikube start --vm-driver=none --kubernetes-version=1.15.4\n"
...
"sles-12.4")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y tar wget gcc git make python curl patch libvirt libvirt-devel libtasn1-devel which
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
"sles-15" | "sles-15.1")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
